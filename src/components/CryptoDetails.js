aaaaaa/* eslint-disable react-hooks/exhaustive-deps */
...
1let { coinId } = useParams();
  ...
  2let { coinId } = useParams();
  let navigate = useNavigate();
  let { getCoinData, coinData: data, currency } = useContext(CryptoContext);
  11let { coinId } = uselet { coinId } = useParams();
  let navigate = useNavigate();let { coinId } = useParams();
  let navigate = useNavigate();
  let { getCoinData, coinData: data, currency } = useContext(CryptoContext);
  let { getCoinData, coinData: data, currency } = useContext(CryptoContext);Params();
  let navigate = useNavigate();
  let { getCoinData, coinData: data, currency } = useContext(CryptoContext);
  1let { coinId } = useParams();
  let navigate = useNavigate();
          ...
          <div className="flex items-center justify-between h-full w-full p-4">
            <div className="flex flex-col w-[45%] h-full pr-2 ">
              <div className="flex w-full items-center">
                ...
                <span
                  className="text-sm
        py-0.5 px-2.5 ml-2 bg-[#ffc757] text-[#ffc757] bg-opacity-25
        rounded uppercase
        "
                >
                  {data.symbol}
          ...
          >
            <div
              className="w-8 h-8 border-4 border-[#ffc757] rounded-full
